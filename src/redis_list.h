...
class List : public Database {
 public:
  ...
  rocksdb::Status Push(const Slice &user_key, const std::vector<Slice> &elems, bool left, int ttl, int *ret);
 ...
 private:
  ...
  rocksdb::Status push(const Slice &user_key, std::vector<Slice> elems, bool create_if_missing, bool left, int ttl, int *ret);
};
}  // namespace Redis
