...
Config::Config() {
  ...
  FieldWrapper fields[] = {
      ...
      {"semi-sync-auto-fail-back", false, new YesNoField(&semi_sync_auto_fail_back, true)},
  ...
  std::map<std::string, CallbackFn> callbacks =
      {
          ...
          {"semi-sync-enable", 
            [this](Server* srv, [[maybe_unused]] const std::string &k, const std::string& v) -> Status {
              ...
              bool is_enable = v == "yes";
              instance.SetSemiSyncEnabled(is_enable);

              int result = 0;
              if (is_enable) {
                result = instance.StartSemiSyncMaster(semi_sync_wait_for_slave_count);
              } else {
                result = instance.CloseSemiSyncMaster();
              }

              if (result == 0) {
                return Status::OK();
              } else {
                return {Status::NotOK, "Failed to set the semi sync enable"};
              }
            }},
